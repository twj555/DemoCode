...
namespace Senparc.Weixin.Entities.Request
{
    ...
    public static class RequestMessageTextExtension
    {
        ...
        /// <returns></returns>
        public static RequestMessageTextKeywordHandler LikeKeyword(this RequestMessageTextKeywordHandler handler, string keyword, Func<IResponseMessageBase> func)
        {
            if (!handler.MatchSuccessed
                && handler.Keyword.Trim().ToUpper().Contains(keyword.ToUpper()))
            {
